...
async function renderEmployeePage() {
  try {
    ...
    const approvedDays = requests
      .filter((request) => request.admin_status === 'approved')
      .reduce((sum, request) => sum + countRequestedDays({
    ...
    app.innerHTML = `
      <header class="topbar">
        <div><h1>Portail Employé</h1><small>${profile.email}</small></div>
        <button id="logout" class="ghost" style="width:auto">Se déconnecter</button>
      </header>
      <section class="grid kpi">
        <article class="card"><h3>Droits</h3><div>${allowance} j</div></article>
        <article class="card"><h3>Pris</h3><div>${approvedDays} j</div></article>
        <article class="card"><h3>Solde</h3><div>${balance} j</div></article>
      </section>
      ...
      <section class="grid two" style="margin-top:16px;">
        <article class="card">
          ...
          <form id="request-form" class="grid">
            ...
            <label><input type="checkbox" name="is_half_day" style="width:auto;margin-right:8px;"/>Demi-journée</label>
      ...
      <section class="card" style="margin-top:16px;">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Type</th><th>Période</th><th>Statut</th><th>Action</th></tr></thead>
            <tbody id="rows"></tbody>
          </table>
        </div>
      </section>`;
    ...
    document.getElementById('request-form').onsubmit = async (event) => {
      ...
      payload.half_day_period = payload.is_half_day ? payload.half_day_period : null;
      payload.status = 'pending_manager';
      payload.manager_status = 'pending';
      payload.admin_status = 'pending';

      if (payload.type === 'other' && !payload.custom_type_text) {
        toast('Précisez le type personnalisé.');
        return;
      }

      const overlap = requests.some((request) => !(payload.end_date < request.start_date || payload.start_date > request.end_date));
      if (overlap) {
        toast('Chevauchement détecté avec une autre demande.');
        return;
      }
    ...
    rows.innerHTML = requests.map((request) => `
      <tr>
        <td>${request.type}${request.custom_type_text ? ` (${request.custom_type_text})` : ''}</td>
        <td>${request.start_date} → ${request.end_date}</td>
        <td>${statusBadge(request.status)}</td>
        <td>
          ${request.status.startsWith('pending') ? `<button class="ghost cancel" style="width:auto" data-id="${request.id}">Annuler</button>` : '—'}
        </td>
      </tr>`).join('');

    rows.querySelectorAll('.cancel').forEach((btn) => {
      btn.onclick = async () => {
        await updateLeaveRequest(btn.dataset.id, { status: 'cancel_pending', comment: 'Demande d\'annulation employé' });
    ...
    app.innerHTML = `<section class="card"><h2>Erreur employé</h2><p>${error.message}</p></section>`;
  }
}
